...
public class CourseServiceImpl implements CourseService {
    ...
    @Override
    public ResultVO<CourseVO> likeCourse(Integer uid, Integer courseId) {
        Course course=courseMapper.selectByPrimaryKey(courseId);
        CourseVO re=new CourseVO(course);
        if(re.isLiked()){
            return new ResultVO<>(Constant.REQUEST_FAIL, "请勿重复点赞。");
        }
        if(courseLikesMapper.insert(courseId,uid) > 0){
            return new ResultVO<CourseVO>(Constant.REQUEST_SUCCESS, "点赞成功。", new CourseVO(course, false, true));
        }
        return new ResultVO<>(Constant.REQUEST_FAIL, "服务器错误");
    }
    ...
    @Override
    public ResultVO<CourseVO> dislikeCourse(Integer uid, Integer courseId) {
        Course course=courseMapper.selectByPrimaryKey(courseId);
        CourseVO re=new CourseVO(course);
        if(!re.isLiked()){
            return new ResultVO<>(Constant.REQUEST_FAIL, "还未被点赞。");
        }
        if(courseLikesMapper.deleteByPrimaryKey(courseId,uid) > 0){
            return new ResultVO<CourseVO>(Constant.REQUEST_SUCCESS, "取消成功。", new CourseVO(course, false, true));
        }
        return new ResultVO<>(Constant.REQUEST_FAIL, "服务器错误");
    }
